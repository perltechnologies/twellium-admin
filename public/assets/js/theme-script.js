...
! function() {
	var _session = sessionStorage.getItem("__THEME_CONFIG__");
	var _local = localStorage.getItem("theme");
	// Merge localStorage theme into sessionStorage config so it persists across sessions
	if (!_session && _local) {
		var _merged = { theme: _local };
		sessionStorage.setItem("__THEME_CONFIG__", JSON.stringify(_merged));
		_session = JSON.stringify(_merged);
	}
	var t = _session,
